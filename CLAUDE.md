...
**Do not call `supabase.auth.getUser()` in middleware on public routes.**
Vercel edge middleware has a hard 25-second timeout (`MIDDLEWARE_INVOCATION_TIMEOUT`). `supabase.auth.getUser()` makes a network round-trip to Supabase's auth service, and when the browser sends a stale or expired session cookie, the JWT refresh path can take long enough to hit this limit — returning a 504 on the landing page. The fix (commit `8a9d0dd`): only call `getUser()` when `pathname.startsWith('/app')`. All other matched routes skip the Supabase round-trip entirely. The session is still refreshed naturally on any `/app/*` navigation. Symptom: 504 on Chrome (which persists cookies across sessions) but not Safari/mobile (which had no stale cookie). Clearing cookies is a user workaround, but the middleware guard is the real fix.
...
**Do not let the Supabase project auto-pause again.**
The Spectr Supabase project (`nlkwoezxicayljemxhma`) is on the free tier, which auto-pauses any project after ~7 days of inactivity. When it pauses, every API call from the frontend and worker fails — the user-facing symptom is `{"error":"user_resolve_failed"}` on the gallery flow (the catch block in `app/api/projects/gallery/route.ts` returns that generic message when `admin.auth.admin.listUsers` errors). The deeper risk: in the May 2026 pause incident, the **entire `public` schema (`projects`, `spec_credits`, `waitlist`) was wiped** on restore — storage buckets and storage RLS policies survived, but every row of application data was gone. The keep-alive cron at `frontend/app/api/keep-alive/route.ts` runs twice weekly (Mon + Thu 12:00 UTC, scheduled via `frontend/vercel.json` `crons` block) and queries `projects` so the DB never goes idle. Do not remove this cron unless the project is upgraded to a paid Supabase plan (which has no auto-pause and daily backups). If you ever see paused-state symptoms again, the schema replay lives in `supabase/schema.sql` + `supabase/migrations/2026-04-15-paywall.sql` + `supabase/migrations/2026-04-16-waitlist-as-main-app.sql`.
